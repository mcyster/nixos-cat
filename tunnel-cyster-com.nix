...
{ config, libs, pkgs, ...}:
{
  users.extraUsers.autossh = {
    # isNormalUser = true;    # uncomment on first setup
    home = "/home/autossh";
  };
  ...
  systemd.services.tunnel-cyster-com = {
    ...
    serviceConfig = {
      User = "autossh";
      ExecStart = "${pkgs.openssh}/bin/ssh -NTC -o ServerAliveInterval=60 -o ExitOnForwardFailure=yes -o StrictHostKeyChecking=no -i %h/.ssh/servername-home-key -R 1128:localhost:22 autossh@cyster.com";
      RestartSec = "15";
      Restart = "always";
    };
